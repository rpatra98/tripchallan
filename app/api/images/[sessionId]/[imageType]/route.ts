...
export const GET = withAuth(
  async (req: NextRequest, context?: { params: Record<string, string> }) => {
    try {
      const { sessionId, imageType } = context?.params || {};
      
      if (!sessionId || !imageType) {
        return NextResponse.json(
          { error: "Session ID and image type are required" },
          { status: 400 }
        );
      }

      // Get index for array-based images (sealing, vehicle, additional)
      const pathParts = imageType.split('/');
      const baseType = pathParts[0];
      const index = pathParts.length > 1 ? parseInt(pathParts[1]) : null;
      ...
      // For array-based images (sealing, vehicle, additional)
      let imagePath: string;
      
      if (index !== null) {
        // For indexed images like sealing/0, vehicle/1, etc.
        imagePath = path.join(uploadsDir, sessionId, baseType, index.toString());
      } else {
        // For single images like driver, gpsImei, vehicleNumber
        imagePath = path.join(uploadsDir, sessionId, baseType);
      }
