...
$insertPosition = $content.IndexOf("  const renderLogDetails = (log: ActivityLog) => {")
$updatedContent = $content.Substring(0, $insertPosition) + $actionIconFunc + "`r`n`r`n" + $content.Substring($insertPosition)
Set-Content -Path $filePath -Value $updatedContent

Write-Host "Restored the getActionIcon function." 